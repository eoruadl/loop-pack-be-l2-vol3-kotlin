...
class PgPaymentClient(
    ...
    @TimeLimiter(name = "pg-payment", fallbackMethod = "requestPaymentFallback")
    override fun requestPayment(request: PgPaymentRequest): CompletableFuture<PgPaymentResponse> {
        return CompletableFuture.supplyAsync {
            try {
                ...
                PgPaymentResponse(pgTransactionId = data.transactionKey)
    ...
    fun requestPaymentFallback(request: PgPaymentRequest, e: Exception): CompletableFuture<PgPaymentResponse> {
        return CompletableFuture.failedFuture(PgPaymentTimeoutException("Circuit breaker open — PG 결제 요청 불가", e))
    }

    @CircuitBreaker(name = "pg-payment", fallbackMethod = "getPaymentFallback")
    @Retry(name = "pg-payment", fallbackMethod = "getPaymentFallback")
    @TimeLimiter(name = "pg-payment", fallbackMethod = "getPaymentFallback")
    ...
    @Suppress("UNUSED_PARAMETER")
    fun getPaymentFallback(pgTxId: String, userId: Long, e: Exception): CompletableFuture<PgPaymentStatusResponse> {
        return CompletableFuture.failedFuture(PgPaymentTimeoutException("Circuit breaker open — PG 상태 조회 불가", e))
    }
    ...
    @TimeLimiter(name = "pg-payment", fallbackMethod = "getPaymentByOrderIdFallback")
        ...
        return CompletableFuture.supplyAsync {
            try {
                ...
                val data = restTemplate.exchange(
                    ...
                    HttpEntity(null, headers),
                    responseType,
                ).body?.data ?: return@supplyAsync null
                val lastTx = data.transactions.lastOrNull() ?: return@supplyAsync null
                PgPaymentStatusResponse(
                ...
                if (e.statusCode.value() == 404) return@supplyAsync null
    ...
    @Suppress("UNUSED_PARAMETER")
    fun getPaymentByOrderIdFallback(orderId: Long, userId: Long, e: Exception): CompletableFuture<PgPaymentStatusResponse?> {
        return CompletableFuture.failedFuture(PgPaymentTimeoutException("Circuit breaker open — PG 주문 기반 조회 불가", e))
    }
